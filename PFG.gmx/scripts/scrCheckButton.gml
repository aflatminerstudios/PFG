/// scrCheckButton(objPlayer_instance, button)
// keys are UP, DOWN, LEFT, RIGHT, BUTTON1, BUTTON2, BUTTON3

var playerInst = argument0;
var button = argument1;

debug_log("scrCheckButton called.");
...
if(playerInst.inputType == INPUT_KEYBOARD) {
  if(keyboard_check(buttonToCheck))
    return true;
} else if(playerInst.inputType == INPUT_XBOX360GAMEPAD) {
  if(gamepad_button_check(playerInst.gamepadNumber, buttonToCheck))
    return true;
}
